...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePrediction(image: UIImage) {
        guard let model = try? VNCoreMLModel(for: animal_cnn_aug().model) else { fatalError("Model not found")
        }
        
        let request = VNCoreMLRequest(model: model) {
            [weak self] request, error in
            
            guard let results = request.resultsa as? [VNClassificationObservation], let firstResult = results.first else {
